...
class TGFeeder:
    ...
    async def _publish_file_task(self, file_path, message):
        """
        Publishes a Celery task for processing the given file path and message.
        """

        try:
            app = Celery(
                        'publisher',
                        broker='pyamqp://guest@localhost//',   # ← point at your RabbitMQ
    ...
    async def get_media(self, obj_json, message, download=False, save_dir=''):
        """
        Downloads media from Telegram messages:
        - Files < size_limit: uses existing _download_media logic and optionally saves to save_dir.
        - Files >= size_limit: downloads, moves to local_storage, and publishes a Celery task.
        """
        if download and message.file:
            ...
            if t_mime_type:
                ...
                tm_type, tm_subtype = None, None

            # Size cutoff (default 10 MB)
            limit = getattr(self, '_size_limit', 10 * 1024 * 1024)

            # --------------------------
            # 1) Small-file branch (< limit)
            # --------------------------
            if message.file.size < limit:
                print("[+] Downloading a file < 10 MB")
                if tm_type in ('application', 'text') and tm_subtype in DOWNLOAD_MIMETYPES.get(tm_type, []):
                    ...
                    if media_content:
                        ...
                        m_type, m_subtype2 = mimetype.split('/')
                        if m_type in ('application', 'text') and m_subtype2 in DOWNLOAD_MIMETYPES.get(m_type, []):
                ...
                if save_dir:
                    ...
                    await message.download_media(file=save_dir, progress_callback=callback_download)

            # --------------------------
            # 2) Large-file branch (>= limit)
            # --------------------------
            else:
                print("[+] Downloading a file > 10 MB")
                if save_dir:
                    ...
                    media_path = await message.download_media(
                        file=save_dir,
                        progress_callback=callback_download
                    )
                else:
                    media_path = await message.download_media(progress_callback=callback_download)

                here = os.path.dirname(__file__)
                local_storage = os.path.join(here, 'local_storage')
                os.makedirs(local_storage, exist_ok=True)
                fname = os.path.basename(media_path)
                target = os.path.join(local_storage, fname)
                shutil.move(media_path, target)

                # attempt to publish, but catch any errors and continue
                text = obj_json.get('data', '') or ''
                try:
                    print(f"[+] Sending to queue the file {target} with the message {text}")
                    await self._publish_file_task(target, text)
                except Exception as e:
                    print(f"[!] Error publishing file task: {e}")
                print(f"[+] Enqueued large file task: {target}")
